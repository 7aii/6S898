...
Comparing the accuracy plot and the spectral norm of the weight matrix plot in Figure 4, we notice that the spectral norm has two inflection points at roughly the same epoch when the model achieves memorization and when it achieves generalization. Between these two points, the spectral norm sharply declines.
...
The plots validate Hypothesis 1. As the weight decay increases, the model achieves generalization faster. However, if weight decay exceeds some threshold between $$0.1$$ and $$0.5$$, then the model cannot achieve generalization at all. Furthermore, by examining the spectral norms of the weight matrices in Figure 6 below, we find that weight decay indeed makes the spectral norm of the weight matrix converge to the limiting spectral norm of the final generalizing circuit faster. If weight decay exceeds the threshold, however, the spectral norm converges quickly to $$0$$ instead, resulting in a decrease of both training and testing accuracy.
...
The plots validate Hypothesis 2. As the fraction of outliers increases, the model achieves generalization slower. However, if the fraction exceeds some threshold between $$0.1$$ and $$0.15$$, then the model cannot achieve generalization at all. We observe the following curious phenomenon, which was also noted in Power et al. (2022)<d-cite key="power2022grokking"></d-cite>. When $$p_t=0.05$$ and $$0.1$$, the training accuracy peaks and then starts to decrease, as the generalizing circuit that is being learned starts to “correct” the outliers. In Figure 7 above, where the weight decay was set to $$0.01$$, the generalizing circuit is never learned and the model achieves memorization even when the fraction of outlier is as high as $$0.2$$. We interpret this result as emphasizing the crucial role that weight decay plays in inducing grokking and, more generally, memorization. We leave clarifying the precise nature of this role as a topic for future research.

## Conclusion

In our exploration of the grokking phenomenon, we have undertaken two empirical experiments that lend support to the learned-representation hypothesis. This theory suggests that grokking is the result of an internal shift in the model from relying on a memorizing circuit to developing a generalizing circuit. Our first experiment underscores the vital role of weight decay in this transformative process, echoing the findings of Verma et al. (2023)<d-cite key="varma2023explaining"></d-cite>. The second experiment adds depth to our understanding by examining how the introduction of outliers to the training data affects the transition.

While our findings contribute to the understanding of grokking, we recognize certain limitations in our approach. Our research parameters were limited to a fixed set of model configurations and did not explore variations in the number of training samples, model complexity (as denoted by width parameter $$W$$), or problem difficulty (reflected in the value of $$n$$ and $$k$$ for the $$(n,k)$$-parity learning task).

One clear conclusion from our work is that further theoretical analysis is needed, particularly to validate the role of weight decay in aiding the transition to generalization. Looking forward, not only should future research aim to address the unexplored aspects of training sample size, model complexity, and problem difficulty, but it should also consider investigating the grokking behavior in deeper models or models with alternative structures. Such exploration could reveal whether the principles of grokking are universally applicable across various model architectures or whether they manifest differently with changes in depth and structure.

By expanding our research to include these additional dimensions, we hope to uncover a more nuanced understanding of the grokking phenomenon. This endeavor will not only help in corroborating our current findings but also in identifying fundamental mechanisms that enable machine learning models to transition from memorization to generalization across diverse tasks and model architectures. The ultimate goal is to harness these insights to develop more advanced and capable learning algorithms.

## Supplementary Findings

While our primary focus remains on the learned-representation explanation of grokking, our research journey led us to uncover several intriguing findings that, while not central to our main hypothesis, present noteworthy insights into the broader context of our study. In this section, we share these additional insights, exploring their implications and how they might inform future research directions in the field of machine learning. 

As we progress to studying the $$(n,k)$$-parity learning task with $$k = 3$$, an interesting pattern emerges. The KDE plots of the hidden layer weights initially exhibit two pronounced peaks when the model entering the memorization phase. This dual-peak feature differs from the $$k = 2$$ case. Nonetheless, as the model progresses towards generalization, both cases show a similar outcome with the weights clustering around zero, as depicted in the below plots.

{% include figure.html path="assets/img/2023-12-12-grokking/fig9.png" class="img-fluid" %}
<div class="caption">
  Figure 9: KDE plots of the weights at various epochs when $k = 3$.
</div>

We might hypothesize that the number of peaks during the memorization state correlates with the number of $$k$$, which could be tied to how the model's complexity increases with higher dimensions of parity. This premise is plausible given that the complexity of the learning task scales with $$k$$, potentially requiring more distinct weight configurations for memorization. Further theoretical investigation is warranted to substantiate this hypothesis. However, ultimately, the generalization phase appears to converge to a similar state across different values of $$k$$, suggesting that despite the initial complexity, the model simplifies to a more uniform and possibly more efficient representation for generalization.
