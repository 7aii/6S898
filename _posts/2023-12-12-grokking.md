...
Comparing the accuracy plot and the spectral norm of the weight matrix plot in Figure 4, we notice that the spectral norm has two inflection points at roughly the same epochs when the model achieves memorization and when it achieves generalization. Between these two points, the spectral norm sharply declines.
